...
export async function remediateHtml(originalHtml) {
  ...
  for (const element of imagesMissingAlt.toArray()) {
    ...
    if (aiResult?.text) {
      ...
      warnings.push(
        'Some images still lack alternative text in this snapshot. Left as an open accessibility issue for manual authoring (remediation path, not the scan-only reporter).'
      );
    }
  }
