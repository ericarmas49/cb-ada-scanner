...
function isLowMemoryHost() {
  return Boolean(process.env.RENDER || process.env.RENDER_SERVICE_ID || process.env.LOW_MEMORY_MODE === 'true');
}
...
export function runAccessibilityDemo({ appRoot, outputRoot = appRoot, url, origin }) {
  ...
  const scanResult = runScanner(scannerRoot, scannerArgs);
  ...
  const inlineArtifacts = process.env.VERCEL
    ? {
        ...
        pdfFileName: reportPdfDataUrl ? pdfFileName : null
      }
    : null;
  ...
  const result = {
    ...
    reportPdfUrl: fs.existsSync(namedPdfPath) ? `${origin}/runs/${runId}/${encodeURIComponent(pdfFileName)}` : null,
