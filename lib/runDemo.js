...
export async function runAccessibilityDemo({ appRoot, url, origin }) {
  ...
  if (beforeScan.status === 1) {
    throw new Error(`Before scan failed: ${beforeScan.stderr || beforeScan.stdout}`);
  }
  ...
  if (!fs.existsSync(beforeReportPath) || !fs.existsSync(snapshotPath) || !fs.existsSync(beforeHtmlPath)) {
    throw new Error('Before scan did not generate the expected files.');
  }
  ...
  if (afterScan.status === 1) {
    throw new Error(`After scan failed: ${afterScan.stderr || afterScan.stdout}`);
  }
  ...
  if (!fs.existsSync(afterReportPath) || !fs.existsSync(afterHtmlPath)) {
    throw new Error('After scan did not generate the expected files.');
  }
